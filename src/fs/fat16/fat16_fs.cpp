...
void Fat16Fs::set_user(uint16_t uid, uint16_t gid) {
    cur_uid_ = uid;
    cur_gid_ = gid;
    user_slot_ = 0;  // single-session: all OFT access goes through slot 0
}
...
ssize_t Fat16Fs::fs_read(int fd, void* buf, size_t len) {
    ...
    // Find file_size from the directory entry (in this fd's parent directory)
...
int Fat16Fs::fs_stat(const char* path, FileStat& out) {
    ...
    // An empty final component means the path resolved to a directory itself
    // (e.g. "/"). The root has no on-disk entry to read, so synthesize its stat.
    if (name.empty()) {
...
// FAT16 has no rwx permission bits, so there is nothing to change. We return 0
// (not -1) on purpose so the shell's `chmod` behaves uniformly across engines;
// on FAT16 it is simply a no-op rather than a reported failure.
...
uint32_t Fat16Fs::get_file_size(uint16_t dir_cluster, uint16_t first_cluster) const {
    // Look only inside the file's own parent directory. Scanning every cluster
    // (and treating file data as directory entries) could alias unrelated bytes
    // as a matching entry. See test FileSizeNotConfusedByDataAliasingDirEntry.
    std::vector<FAT16DirEntry> entries;
